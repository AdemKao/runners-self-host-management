class Runnerctl < Formula
  ...
  test do
    assert_equal "0.3.2", shell_output("#{bin}/runnerctl version").strip
    assert_match "AI AGENT", shell_output("#{bin}/runnerctl --help")
    assert_match '"agent_ready": true', shell_output("#{bin}/runnerctl agent --json")
    assert_match '"current_version":"0.3.2"', shell_output("RUNNERCTL_LATEST_VERSION=0.3.2 RUNNERCTL_INSTALL_METHOD=homebrew #{bin}/runnerctl upgrade --check --json")
  end
end
