class Runnerctl < Formula
  ...
  def install
    ...
    %w[runnerctl-frontend runnerctl-base runnerctl-base-v05 runnerctl-base-legacy runnerctl-core runnerctl-core-legacy runnerctl-cleanup runnerctl-host runnerctl-ci runnerctl-hooks runnerctl-queue runnerctl-queue-legacy runnerctl-scheduler runnerctl-scheduler-core runnerctl-notify runnerctl-notify-provider-telegram runnerctl-notify-provider-line runnerctl-notify-provider-webhook].each do |name|
      chmod 0755, libexec/name
    end

    (libexec/"bin").mkpath
    %w[runnerctl-host runnerctl-ci runnerctl-hooks runnerctl-queue runnerctl-queue-legacy runnerctl-scheduler runnerctl-scheduler-core runnerctl-notify runnerctl-notify-provider-telegram runnerctl-notify-provider-line runnerctl-notify-provider-webhook].each do |name|
  ...
  test do
    assert_equal "0.6.0", shell_output("#{bin}/runnerctl version").strip
    ...
    assert_match '"current_version":"0.6.0"', shell_output("RUNNERCTL_LATEST_VERSION=0.6.0 RUNNERCTL_INSTALL_METHOD=homebrew #{bin}/runnerctl upgrade --check --json")
  end
end