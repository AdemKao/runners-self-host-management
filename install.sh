...
install_extra(){ local source="$1" name="$2"; [[ -f "$source" ]] || die "missing v0.8 component: $source"; mkdir -p "$LIBEXEC_DIR"; install -m 0755 "$source" "$LIBEXEC_DIR/$name"; }
local_checkout_ready(){
  [[ -n "$SCRIPT_DIR" && -f "$SCRIPT_DIR/install-legacy.sh" && -f "$SCRIPT_DIR/runnerctl-base-v07" && -f "$SCRIPT_DIR/runnerctl-base-v06" && -f "$SCRIPT_DIR/runnerctl-base-v05" && -f "$SCRIPT_DIR/runnerctl-base-legacy" && \
     -f "$SCRIPT_DIR/bin/runnerctl-v07" && -f "$SCRIPT_DIR/bin/runnerctl-legacy" && -f "$SCRIPT_DIR/bin/runnerctl-queue-legacy" && -f "$SCRIPT_DIR/bin/runnerctl-scheduler" && \
     ...
     -f "$SCRIPT_DIR/bin/runnerctl-bot-controller.py" && -f "$SCRIPT_DIR/bin/runnerctl-monitor" ]]
}
...
install_v08_components(){
  local tmp="$1" base name
  local names=(runnerctl-base-v07 runnerctl-base-v06 runnerctl-base-v05 runnerctl-base-legacy runnerctl-core-v07 runnerctl-core-legacy runnerctl-queue-legacy runnerctl-scheduler runnerctl-scheduler-core runnerctl-notify runnerctl-notify-provider-telegram runnerctl-notify-provider-line runnerctl-notify-provider-webhook runnerctl-bot runnerctl-bot-controller.py runnerctl-monitor)
  if local_checkout_ready; then
    install_extra "$SCRIPT_DIR/runnerctl-base-v07" runnerctl-base-v07
    ...
    install_extra "$SCRIPT_DIR/bin/runnerctl-v07" runnerctl-core-v07
    ...
    install_extra "$SCRIPT_DIR/bin/runnerctl-monitor" runnerctl-monitor
    ...
    curl -fsSL "https://raw.githubusercontent.com/$REPO/$REF/runnerctl-base-v07" -o "$tmp/runnerctl-base-v07"
    ...
    curl -fsSL "https://raw.githubusercontent.com/$REPO/$REF/bin/runnerctl-v07" -o "$tmp/runnerctl-core-v07"
    ...
    curl -fsSL "https://raw.githubusercontent.com/$REPO/$REF/bin/runnerctl-monitor" -o "$tmp/runnerctl-monitor"
    for name in "${names[@]}"; do install_extra "$tmp/$name" "$name"; done
  fi
  mkdir -p "$LIBEXEC_DIR/bin"
  for name in runnerctl-notify runnerctl-notify-provider-telegram runnerctl-notify-provider-line runnerctl-notify-provider-webhook runnerctl-bot runnerctl-bot-controller.py runnerctl-monitor; do
...
main(){
  ...
  install_v08_components "$TMP_DIR"
  cleanup
  info "Installed v0.8 monitor, bot, notification, and scheduler components to $LIBEXEC_DIR"
  printf '\nAuthoritative outcome monitoring:\n  runnerctl monitor status --json\n  runnerctl monitor enable --interval 60 --retention 200\n  runnerctl monitor jobs --limit 20 --json\n  runnerctl monitor failures --limit 20 --json\n\nSee docs/monitoring.md for final-conclusion semantics, polling, retention, notifications, and supervision.\n'
}
main "$@"