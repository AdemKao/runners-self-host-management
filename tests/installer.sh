...
RUNNERCTL_LATEST_VERSION="$VERSION" RUNNERCTL_INSTALL_METHOD=shell "$cli" upgrade --check --json | node -e 'const fs=require("fs");const x=JSON.parse(fs.readFileSync(0,"utf8"));if(x.current_version!=="0.7.4"||x.update_available)process.exit(1)'
...
echo "v0.7.4 installer tests passed"
