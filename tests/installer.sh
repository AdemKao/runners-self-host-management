...
[[ "$($cli version)" == "$VERSION" ]]
for f in runnerctl-base runnerctl-base-v07 runnerctl-base-v06 runnerctl-base-v05 runnerctl-base-legacy runnerctl-core runnerctl-core-v07 runnerctl-core-legacy runnerctl-cleanup runnerctl-host runnerctl-ci runnerctl-hooks runnerctl-queue runnerctl-queue-legacy runnerctl-scheduler runnerctl-scheduler-core runnerctl-notify runnerctl-notify-provider-telegram runnerctl-notify-provider-line runnerctl-notify-provider-webhook runnerctl-bot runnerctl-bot-controller.py runnerctl-monitor; do
...
"$cli" monitor --help >"$tmp/monitor-help"; grep -q 'final-outcome monitoring' "$tmp/monitor-help"
"$cli" monitor status --json | node -e 'const fs=require("fs");const x=JSON.parse(fs.readFileSync(0,"utf8"));if(x.history_count!==0||x.enabled!==false)process.exit(1)'
"$cli" monitor jobs --json | node -e 'const fs=require("fs");const x=JSON.parse(fs.readFileSync(0,"utf8"));if(!Array.isArray(x)||x.length!==0)process.exit(1)'
"$cli" --help >"$tmp/root-help"; grep -q 'GitHub-native scheduling' "$tmp/root-help"; grep -q 'Notifications and integrations' "$tmp/root-help"; grep -q 'Read-only Bot/API controller' "$tmp/root-help"; grep -q 'Authoritative job outcome monitoring' "$tmp/root-help"
if command -v python3 >/dev/null 2>&1; then
  "$cli" bot --help >"$tmp/bot-help"; grep -q 'Read-only Telegram, LINE, and HTTP API controller' "$tmp/bot-help"
  "$cli" bot doctor --json | python3 -c 'import json,sys; x=json.load(sys.stdin); assert x["version"]=="0.8.0" and x["read_only"] is True'
fi
