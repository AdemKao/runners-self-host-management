...
[[ "$(bash "$ROOT/runnerctl" version)" == "0.3.1" ]]
...
node -e 'const fs=require("fs"); const x=JSON.parse(fs.readFileSync(0,"utf8")); if(!x.agent_ready || x.version!=="0.3.1" || !x.commands["upgrade --check"]) process.exit(1)' < <(bash "$ROOT/runnerctl" agent --json)

node -e 'const fs=require("fs"); const x=JSON.parse(fs.readFileSync(0,"utf8")); if(x.current_version!=="0.3.1" || x.latest_version!=="0.3.2" || !x.update_available || x.install_method!=="shell") process.exit(1)' \
...
node -e 'const fs=require("fs"); const x=JSON.parse(fs.readFileSync(0,"utf8")); if(x.update_available) process.exit(1)' \
  < <(RUNNERCTL_LATEST_VERSION=0.3.1 RUNNERCTL_INSTALL_METHOD=shell bash "$ROOT/runnerctl" upgrade --check --json)
...
node -e 'const fs=require("fs"); const x=JSON.parse(fs.readFileSync(0,"utf8")); if(x.version!=="0.3.1" || !x.dependencies.gh) process.exit(1)' < <(run doctor --json)
...
RUNNERCTL_TEST_BREW_LOG="$tmp/brew.log" PATH="$tmp/bin:$PATH" RUNNERCTL_LATEST_VERSION=0.3.2 RUNNERCTL_INSTALL_METHOD=homebrew \
...
[[ "$($tmp/local/bin/runnerctl version)" == "0.3.1" ]]
...
[[ -f "$tmp/dist/runnerctl-0.3.1.tar.gz" ]]

if command -v sha256sum >/dev/null 2>&1; then
  (cd "$tmp/dist" && sha256sum -c runnerctl.sha256 runnerctl-core.sha256 runnerctl-0.3.1.tar.gz.sha256 >/dev/null)
elif command -v shasum >/dev/null 2>&1; then
  (cd "$tmp/dist" && shasum -a 256 -c runnerctl.sha256 runnerctl-core.sha256 runnerctl-0.3.1.tar.gz.sha256 >/dev/null)
fi
