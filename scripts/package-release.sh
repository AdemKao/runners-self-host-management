...
install -m 0755 "$ROOT/bin/runnerctl-notify" "$DIST_DIR/runnerctl-notify"
install -m 0755 "$ROOT/bin/runnerctl-notify-provider-telegram" "$DIST_DIR/runnerctl-notify-provider-telegram"
install -m 0755 "$ROOT/bin/runnerctl-notify-provider-line" "$DIST_DIR/runnerctl-notify-provider-line"
install -m 0755 "$ROOT/bin/runnerctl-notify-provider-webhook" "$DIST_DIR/runnerctl-notify-provider-webhook"
for f in runnerctl runnerctl-cleanup runnerctl-host runnerctl-ci runnerctl-hooks runnerctl-queue runnerctl-queue-legacy runnerctl-scheduler runnerctl-scheduler-core runnerctl-notify runnerctl-notify-provider-telegram runnerctl-notify-provider-line runnerctl-notify-provider-webhook; do
  src="$ROOT/bin/$f"; [[ "$f" == runnerctl ]] && src="$ROOT/bin/runnerctl"; [[ -f "$src" ]] && install -m 0755 "$src" "$DIST_DIR/bin/$f"
done
# Source-compatible tarball plus compatibility implementations used by v0.6 wrappers.
tar -czf "$DIST_DIR/runnerctl-${VERSION}.tar.gz" -C "$ROOT" \
  runnerctl runnerctl-base runnerctl-base-v05 runnerctl-base-legacy \
